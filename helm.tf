...
resource "helm_release" "jenkins" {
  ...
  set {
    ...
    value = "${azurerm_dns_a_record.jenkins.name}.${azurerm_dns_zone.jungo.name}"
  }
}
...
resource "helm_release" "istio" {
  ...
  depends_on = [azurerm_kubernetes_cluster.aks, azurerm_public_ip.ingress_ip, kubernetes_cluster_role_binding.tiller_sa_cluster_admin_rb, helm_release.ingress, helm_release.istio_init]
...
/*resource "helm_release" "spinnaker" {
  ...
  values = [
    "${file("spinnaker.values.yaml")}"
  ]
  set {
    name  = "ingress.host"
    value = "${azurerm_dns_a_record.spinnaker_ingress.name}.${azurerm_dns_zone.jungo.name}"
  }
  set {
    name  = "ingressGate.host"
    value = "${azurerm_dns_a_record.spinnaker_ingress_gate.name}.${azurerm_dns_zone.jungo.name}"
  }
}*/